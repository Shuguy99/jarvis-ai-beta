...
const COMMANDS: QuickCommand[] = [
  ...
  { icon: Wand2, label: "Создай картинку", prompt: "Futuristic holographic HUD interface with glowing cyan lines, dark background, sci-fi aesthetic, high quality", isImageGen: true },
  { icon: Palette, label: "Арт", prompt: "Cyberpunk cityscape at night with neon signs, rain reflections, cinematic lighting, digital art, high quality", isImageGen: true },
];

export function QuickCommands({ onPick, onImageGen }: { onPick: (prompt: string) => void; onImageGen?: (prompt: string) => void }) {
  return (
    <div className="flex flex-wrap gap-1.5">
      {COMMANDS.map((c) => (
        <motion.button
          key={c.label}
          onClick={() => { playSound("click"); if (c.isImageGen && onImageGen) onImageGen(c.prompt); else onPick(c.prompt); }}
